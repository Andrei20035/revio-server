...
class PushDispatchServiceTest {
    ...
    fun `a 404 with errorCode UNREGISTERED is Terminal UNREGISTERED`() {
        ...
        """.trimIndent()
        val result = classifyResponse(json = json, isSuccess = false, statusCode = 404, retryAfterHeader = null, responseText = body)
    ...
    fun `a 400 with errorCode INVALID_ARGUMENT is Terminal INVALID_ARGUMENT`() {
        ...
        """.trimIndent()
        val result = classifyResponse(json = json, isSuccess = false, statusCode = 400, retryAfterHeader = null, responseText = body)
    ...
    fun `a 429 is Retriable and carries the Retry-After header when present`() {
        ...
        val result = classifyResponse(json = json, isSuccess = false, statusCode = 429, retryAfterHeader = "30", responseText = body)
    ...
    fun `a 500 is Retriable with no Retry-After header`() {
        ...
        val result = classifyResponse(json = json, isSuccess = false, statusCode = 500, retryAfterHeader = null, responseText = body)
    ...
    fun `a 503 is Retriable`() {
        ...
        val result = classifyResponse(json = json, isSuccess = false, statusCode = 503, retryAfterHeader = null, responseText = body)
    ...
    fun `an unrecognized error status is Unknown, body preserved as-is`() {
        ...
        val result = classifyResponse(json = json, isSuccess = false, statusCode = 403, retryAfterHeader = null, responseText = body)
        assertEquals(FcmSendResult.Unknown(403, body), result)
    }

    @Test
    fun `a completely unparseable error body still classifies by HTTP status alone`() {
        val result = classifyResponse(json = json, isSuccess = false, statusCode = 503, retryAfterHeader = null, responseText = "<html>gateway timeout</html>")
        assertEquals(FcmSendResult.Retriable(503), result)
    }
}
