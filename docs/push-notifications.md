...
| `CRON_SECRET` | Shared secret required by `X-Cron-Secret`-protected internal endpoints (e.g. `/api/internal/notifications/inactivity`). Missing → those endpoints fail closed (401). | Endpoints reject every request. |
